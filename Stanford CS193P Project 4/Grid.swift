...
struct Grid
{
    ...
    private mutating func updateCellFrames(to cellSize: CGSize) {
		...
		let insetValue = cellSize.width * 5 / 100 // 5 % of width

        if cellCount > 0 {
            for _ in 0..<cellCount {
				let newRect = CGRect(origin: origin, size: cellSize).insetBy(dx: insetValue, dy: insetValue)
				cellFrames.append(newRect)
